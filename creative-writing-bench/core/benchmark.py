...
def compute_benchmark_results_creative(runs, run_key, runs_file, negative_criteria):
        ...
        for prompt_id, t_info in p_dict.items():
            ...
            if t_info.get("status") in ["completed", "judged"]:
                ...
                if has_scores:
                    ...
                    logging.warning(
                        f"Task {run_key}/{i_str}/{prompt_id} has status '{t_info.get('status')}' but no judge_scores found. Excluding from final results."
                    )

    if not completed_tasks:
        logging.warning(
            f"No completed tasks with scores found for run {run_key}. No final results computed."
        )
        ...
        bench_results["bootstrap_analysis"] = {
            "error": "No completed tasks with scores"
        }
    ...
    # 1) Summarize
    summary_result = compute_single_benchmark_score_creative(
        completed_tasks, negative_criteria
    )
    creative_score_0_20 = summary_result["overall_score"]
    eqbench_creative_score = summary_result["eqbench_creative_score"]

    # 2) Bootstrap
    boot_stats = bootstrap_benchmark_stability_creative(
        completed_tasks, negative_criteria
    )
    ...
    logging.info(
        f"Creative benchmark summary => Score(0-20)={creative_score_0_20:.2f}, eqbench_creative(0..100)={eqbench_creative_score:.2f}"
    )
    if "error" not in boot_stats:
        logging.info(
            f"Bootstrap 95% CI: ({boot_stats['ci_lower']:.2f}, {boot_stats['ci_upper']:.2f})"
        )


def pick_best_iteration_for_each_prompt_model(
    run_data, negative_criteria
) -> Dict[str, Any]:
    """
                ...
                for metric, val in j_scores.items():
                    if isinstance(val, (int, float)):
                        try:
                            ...
                            logging.warning(
                                f"Non-numeric score value '{val}' for metric '{metric}' in task {model_name}/{prompt_id}/iter{iteration_idx}. Skipping."
                            )
                            continue
...
def run_eq_bench_creative(
    ...
    if run_key not in runs:
        ...
        if "start_time" not in runs[run_key]:
            update_run_data(
                runs_file, run_key, {"start_time": datetime.now().isoformat()}
            )
    ...
    if not os.path.exists(creative_prompts_file):
        raise FileNotFoundError(
            f"Creative prompts file not found: {creative_prompts_file}"
        )
    # 2. Load Prompts & Criteria
    with open("debug_log.txt", "a") as f:
        f.write("DEBUG: Loading prompts\n")
    creative_prompts = load_json_file(creative_prompts_file)

    # 3. Build API Clients
    with open("debug_log.txt", "a") as f:
        f.write("DEBUG: Building API clients\n")
    api_clients = {
        "test": APIClient(model_type="test"),
        "judge": APIClient(model_type="judge"),
    }
    with open("debug_log.txt", "a") as f:
        f.write("DEBUG: API clients built\n")
    ...
    if redo_judging:
        ...
        run_data_for_redo = load_json_file(runs_file).get(
            run_key, {}
        )  # Load fresh data
        ...
        for i_str, p_dict in c_tasks_for_redo.items():
            for prompt_id, c_dict in p_dict.items():
                if c_dict.get("status") in ["completed", "judged"]:
                    ...
                    tasks_updated = True
                    logging.debug(
                        f"Marking task for reset: iteration={i_str}, prompt_id={prompt_id}"
                    )

        # Perform batch update to the file
        if tasks_updated:
            update_run_data(
                runs_file, run_key, {"creative_tasks": tasks_to_update_in_file}
            )
            logging.info(
                "Completed resetting judge data in file due to --redo-judging flag."
            )
        else:
            logging.info(
                "No tasks found with 'completed' or 'judged' status to reset for --redo-judging."
            )

    # --- Figure out tasks: Create or load task objects into memory ---
    run_data = load_json_file(runs_file).get(
        run_key, {}
    )  # Load potentially updated data
    ...
    with open("debug_log.txt", "a") as f:
        f.write(f"DEBUG: Processing {len(creative_prompts)} prompts\n")
        ...
        for i in range(1, iterations + 1):
            ...
            if c_data and c_data.get("test_model") == test_model:
                ...
                try:
                    ...
                    logging.error(
                        f"Failed to load task from dict for iter={i_str}, prompt={prompt_key}: {e}. Skipping task.",
                        exc_info=True,
                    )
    ...
    # 5.1 Ensure judge_model is updated on all tasks (especially resumed ones)
    for task in tasks_to_run:
        if task.judge_model != judge_model:
            logging.info(
                f"Updating judge_model for task {task.prompt_id} (iter {task.iteration_index}): {task.judge_model} -> {judge_model}"
            )
            task.judge_model = judge_model

    logging.info(
        f"Total task objects prepared: {len(tasks_to_run)} (across {iterations} iteration(s))"
    )

    # --- 1) Generate (if needed) ---
    # 6. Generate Creative Pieces (Parallel)
    tasks_needing_generation = [
        t for t in tasks_to_run if t.status not in ["generated", "completed", "judged"]
    ]
    with open("debug_log.txt", "a") as f:
        f.write(f"DEBUG: Tasks needing generation: {len(tasks_needing_generation)}\n")

    if tasks_needing_generation:
        logging.info(
            f"Found {len(tasks_needing_generation)} tasks requiring generation."
        )
        with open("debug_log.txt", "a") as f:
            f.write("DEBUG: Starting sequential generation loop\n")

        for i, task in enumerate(tasks_needing_generation):
            with open("debug_log.txt", "a") as f:
                f.write(
                    f"DEBUG: Processing task {i+1}/{len(tasks_needing_generation)}\n"
                )
            try:
                task.generate_creative_piece(api_clients["test"], runs_file, run_key)
            except Exception as e:
                with open("debug_log.txt", "a") as f:
                    f.write(f"DEBUG: Task failed: {e}\n")
    ...
    for (
        task_obj
    ) in tasks_to_run:  # Iterate through the objects potentially updated by generation
        needs_judging = False
        ...
        if (
            redo_judging and not needs_judging
        ):  # Only check file if not already marked by 'generated' status
            ...
            if saved_status == "generated":  # Check if file was reset correctly
                needs_judging = True
                logging.debug(
                    f"Task {prompt_id} (Iter {i_str}) marked for judging via redo file check."
                )
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for fut in tqdm(
                ...
                try:
                    ...
                    logging.error(
                        f"Error judging task {task.prompt_id} (iter {task.iteration_index}): {e}",
                        exc_info=True,
                    )
    ...
    compute_benchmark_results_creative(
        final_runs_data, run_key, runs_file, negative_criteria
    )
    ...
    if run_elo:
        ...
        try:
            ...
            bench_results = results_dict.get(
                "benchmark_results", {}
            )  # Get latest benchmark results
            bench_results["elo_raw"] = elo_raw
            bench_results["elo_normalized"] = elo_norm
            results_dict["benchmark_results"] = (
                bench_results  # Put updated benchmark results back
            )
            update_run_data(
                runs_file, run_key, {"results": results_dict}
            )  # Save updated results

            logging.info(
                f"ELO scores for {test_model}: Raw: {elo_raw}, Normalized: {elo_norm}"
            )

        except FileNotFoundError:
            logging.error(
                "ELO analysis skipped: elo_results.json not found. Was ELO run successful?"
            )
