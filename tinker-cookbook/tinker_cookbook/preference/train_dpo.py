...
def create_dpo_clients(
    ...
    if config.reference_model_name is not None:
        # User explicitly provided a reference model, it could be a base model string or a checkpoint path
        try:
            # First assume it's a LoRA checkpoint path
            reference_training_client = service_client.create_lora_training_client(
                base_model=config.model_name, rank=config.lora_rank
            )
            reference_training_client.load_state(config.reference_model_name).result()
            reference_client = reference_training_client.save_weights_and_get_sampling_client("reference")
        except Exception:
            # Fallback assuming it's a raw Hugging Face base model string
            reference_client = service_client.create_sampling_client(base_model=config.reference_model_name)
    elif config.load_checkpoint_path is not None:
        # Reference should be the frozen prior model state that we are starting training from
        reference_training_client = service_client.create_lora_training_client(
            base_model=config.model_name, rank=config.lora_rank
        )
        reference_training_client.load_state(config.load_checkpoint_path).result()
        reference_client = reference_training_client.save_weights_and_get_sampling_client("reference")
    else:
        # Fallback to the base model string
        reference_client = service_client.create_sampling_client(base_model=config.model_name)
...
def compute_dpo_loss(
    ...
    Args:
        ...
        chosen_lengths: Lengths of the chosen sequences (in tokens)
        dpo_beta: DPO beta parameter
        rpo_alpha: RPO alpha parameter for the NLL loss term
    ...
    # Compute RPO NLL loss if applicable
    nll_loss = 0.0
    if rpo_alpha > 0.0:
        # Normalize the log probability of the chosen sequence by its length
        chosen_nll = torch.stack(
            [-lp / length for lp, length in zip(chosen_logprobs, chosen_lengths, strict=True)]
        )
        nll_loss = chosen_nll.mean()
    ...
    if rpo_alpha > 0.0:
        metrics["nll_loss"] = nll_loss.item() if isinstance(nll_loss, torch.Tensor) else nll_loss
...
def do_update(
    ...
    def dpo_loss_fn(
        ...
        for i in range(len(chosen_data)):
            ...
            # The length of the sequence is the number of non-zero weights
            chosen_length = int((chosen_weights > 0).sum().item())
            chosen_lengths.append(max(1, chosen_length))  # Guard against zero length
